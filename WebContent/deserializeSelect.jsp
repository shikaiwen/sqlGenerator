...
function sendJson(){
	 var jsonStr = getJson();
	 $.post("sqlGen/handleJson",{json:jsonStr},function(datas){
			alert("ok");
...
function toStandardJavaName(str){
	if(str.split("_").length == 1){
		...
		for(var j = 0;j<arr.length;j++){
			if(j == 0){
				...
				resultStr += arr[j].charAt(0).toUpperCase() + arr[j].substring(1,arr[j].length);
			}
		}
		return resultStr;
	}
...
/////////////////////////////////////////////重写/////////////////////////////////////////////////////////////////////////////////
//当前工作的sqlArea，默认为第一个，当点击其他的sqlArea时，这个对象也进行切换
...
//添加tableBlock,id 为当前tableBlock的个数加1
...
function addRelationBlock(){
 ...
 $(function(){
	//获取反序列化数据
	$.get("sqlGen/recoverSql",function(datas){
...
<div name="relationBlock" style="float:left;border:1px solid yellow">
	左联<input type="checkbox" id="leftJoin" name="relationType"  value="1"/>
	内联<input type="checkbox" id="innerJoin" name="relationType" value="2"/>
	右联<input type="checkbox" id="rightJoin" name="relationType" value="3"/><br>
...
<!-- sqlArea模板 -->
<div id="sqlAreaTemplate" style="display:none;">
	<div id="sqlArea1" name="sqlArea">
	<div name="tableBlock1" style="border:1px dotted pink;width:400px;float:left;">
		<table id="tableInfoArea" cellspacing="0">
			<tr>
			<td><input type="button" name="getTableInfoBtn" onclick="getTableInfo(this)" value="获取表信息"/></td>
			<td>表名:<input type="text" id="dbTableName" size="8"/></td>
			<td>别名:</td><td><input type="text" id="dbTableNameAlias" size="8"/></td>
			<td><a status="1" href="#" onclick="openOrClose(this)">收起</a></td>
			</tr>
		</table>
		<br>
