...
currentNode = window.parent.currentNode;
//当前工作的sqlArea
...
<c:choose>
	...
	<c:otherwise>
		...
		$(function(){
			//加载完毕
			$.ajax({
				url:'<%=base%>/packageController/getJsonFile',
				type:'POST',
...
<%
	...
	if(fileName != null){
		%>
		//如果是添加 完成之后的刷新，是从这里反序列化的,也就是从 服务器端返回 数据
		$(function(){
			deserializeData(<%=jsonStr%>);
			refreshParentNode();
		});
...
function getTableInfo(clickedButton){
	...
	$.ajax({
		...
		url:'<%=base%>/centerController/getTableInfo',
		success:function(datas){
...
function getJson(){
	...
	var tableBlockArray = currentSqlArea.find("[name='tableBlock']");
 	...
 	for(var v = 0;v< relationBlockArray.length; v++){
 		...
 		var joinType = $(relationBlock).find("[name='joinType']:checked").val();
...
function realSend(){
//	alert(JSON.stringify(currentNode));return;
	 var sendData = {};
	 sendData['jsonStr'] = getJson();
	 sendData['fileName'] = currentNode['id'];

	 $.post("<%=base%>/packageController/updateJson",sendData,function(datas){
			//alert(datas);
...
function initTableBlock(tableBlockObj){
	...
	$.ajax({
		...
		url:'<%=base%>/centerController/getTableInfo',
		success:function(datas){
			//alert("获取表"+tableName +"数据:"+datas);
			if(!datas) alert("获取表"+tableName+"数据失败");
			var jsonObj = $.parseJSON(datas);
...
function generateSql(){
	$.post("sqlGeneratorController/generateSql",{'id':currentNode['id']},function(datas){
...
<br>
 <style>
    ...
    <div id="centerArea" data-options="region:'center'">
		  ...
		  <input type="button" onclick="realSend()" value="保存Json"/>
		<input type="button" onclick="generateSql()" value="生成Sql"/>
		<div id="workArea">
		...
		<div name="relationBlock" style="float:left;border:1px solid yellow">
			左联<input type="checkbox" id="leftJoin" name="joinType"  value="1"/>
			内联<input type="checkbox" id="innerJoin" name="joinType" value="2"/>
			右联<input type="checkbox" id="rightJoin" name="joinType" value="3"/><br>
